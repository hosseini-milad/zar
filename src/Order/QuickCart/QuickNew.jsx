...
function QuickNew(props) {
  ...
  const ParamType =
    (selectedItem &&
      ...
      selectedItem.parameters.length) % 2;
  ...
  return (
    <tr className={`input-tr ${props.tab ? "pay-tr" : ""}`}>
      ...
      <td data-cell="عیار">
        {props.tab ? (
          selectedItem && selectedItem.ayar ? (
            selectedItem.ayar
          ) : (
            <div className="code-input-wrapper new-input">
              <input
      ...
      <td
        data-cell="مبلغ واحد"
        className={selectedItem && selectedItem.isCoin ? "long-td" : ""}
      >
        {selectedItem
          ? normalPriceCount(
              selectedItem.priceDetail && selectedItem.priceDetail.unitPrice
            )
          : ""}
      </td>

      {props.tab ? (
        selectedItem && !selectedItem.isCoin ? (
          <td data-cell="مبلغ کل" className={ParamType ? "long-td" : ""}>
            {selectedItem ? (
              TotalPrice ? (
                normalPriceCount(TotalPrice)
              ) : (
                <button className="total-btn" onClick={FindTotal}>
                  محاسبه قیمت
                </button>
              )
            ) : (
        ...
        <td data-cell="مبلغ کل">
          {selectedItem ? normalPriceCount(selectedItem.price) : ""}
        </td>
      )}
      <td>
        <div className="more-btn">
          <i className="fa-solid fa-comment" onClick={() => setShowDesc(1)}></i>
          {props.tab ? (
            <i className="fa-solid fa-plus" onClick={addPayItem}></i>
          ) : (
            <i
