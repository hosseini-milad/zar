...
router.post('/list-product', async (req,res)=>{
    ...
    const isFeature = req.body.isFeature?req.body.isFeature:false
    try{
...
router.post('/fetch-product', async (req,res)=>{
    ...
    try{
        ...
        productData.color=productData.color?productData.color:"نامشخص"
        productData.colorCode=productData.colorCode?productData.colorCode:"#eee"
...
router.post('/update-discount',auth,jsonParser, async (req,res)=>{
    ...
    try{
        ...
        for(var i=0;i<myCart.length;i++){
            var cartItem = myCart[i]//UpdateCartQuery
            const productItem = await products.findOne({sku:cartItem.sku})
            var priceDetail = CalcPrice(productItem,priceRaw,TAX&&TAX.percent,discount)
            //console.log(priceDetail.roundPrice)
            await cart.updateOne({_id:ObjectID(cartItem._id)},
                {$set:{discount:discount,priceDetail:priceDetail.priceDetail,
                    fullPrice:priceDetail.priceDetail.roundPrice}})
...
router.post('/cart-to-faktor',auth,jsonParser, async (req,res)=>{
    ...
    try{
        ...
        for(var i=0;i<(cartDetail.cart&&cartDetail.cart.length);i++){
            var cartItem = cartDetail.cart[i]
            const productDetail = await products.findOne({sku:cartItem.sku})
            const priceData = CalcPrice(productDetail,priceRaw,TAX&&TAX.percent,cartItem.discount)
            const fullPrice = priceData.price
            totalFull+=fullPrice
            const price = cartItem.isReserve?
                (parseFloat(PRE&&PRE.percent)*fullPrice/100):fullPrice
...
router.post('/fetch-faktor',auth, async (req,res)=>{
    ...
    try{
        ...
        const FaktorItems = await faktorItems.find({faktorNo:faktorNo}).sort({purchase:-1}).lean()
        const saleItems = []
        const purchaseItems = []
        var index =1
